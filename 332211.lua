...
local Dbg = getgenv().LabaDbg or false
local function dbg(...) if Dbg then warn("[Dbg]", ...) end end
...
tool=pt(pr)
				if tool then
					...
					lastType=pt2
					dbg("clean equips", pt2, tool.Name, "firePending=", tostring(firePending))
				end
			end
...
local function doFireWork()
	...
	local pcs=b:FindFirstChild("PCS");if pcs then
		for _,pc in ipairs(pcs:GetChildren())do if pc:IsA("Model")and pc:GetAttribute("FireActive")==true then dbg("firework PC fire on", pc.Name);return true end end
	end
	...
	if cs then
		for _,r in ipairs({cs:FindFirstChild("ActiveFires"),cs:FindFirstChild("Fires")})do
			if r then for _,p in ipairs(r:GetDescendants())do if p:IsA("ProximityPrompt")and p.Enabled and ct(p)=="fire" then dbg("firework cleaning fire on", r.Name);return true end end end
		end
	end
	return false
end
...
task.spawn(function()
	while alive() do
		...
		if AE and doFireWork() then
			firePending = true
			billBusy = true
			dbg("fire loop preempts (fire active)")
			pcall(doFire)
			billBusy = false
			firePending = false
		else
