...
local function setupAutoExecute()
	if getgenv().AutoExecute and not getgenv().AutoExec then
		...
		queue_on_teleport([[
			repeat task.wait() until game:IsLoaded()
			task.wait(5)
			loadstring(game:HttpGet("[https://raw.githubusercontent.com/Crelle1130/123asd/refs/heads/main/Tryaot.lua](https://raw.githubusercontent.com/Crelle1130/123asd/refs/heads/main/Tryaot.lua)", true))()
		]])
	end
end
...
local repo = "[https://raw.githubusercontent.com/deividcomsono/Obsidian/main/](https://raw.githubusercontent.com/deividcomsono/Obsidian/main/)"
...
local QuestsGroup = Tabs.Misc:AddLeftGroupbox("Quests")
local ShopGroup = Tabs.Misc:AddRightGroupbox("Shop & Boosts")
...
AutoStartGroup:AddButton({
	Text = "Join Discord",
	Func = function()
		setclipboard("[https://discord.gg/N83Tn2SkJz](https://discord.gg/N83Tn2SkJz)")
		Library:Notify({
			Title = "Discord",
...
Toggles.AutoSellPerksToggle:OnChanged(function()
	getgenv().AutoSellPerks = Toggles.AutoSellPerksToggle.Value
	if getgenv().AutoSellPerks then
		if game.PlaceId ~= 14916516914 then return end
		task.spawn(function()
			while getgenv().AutoSellPerks do
				local plrData = GetPlayerData()
				if not plrData or not plrData.Slots then task.wait(1) continue end
				local slotIndex = lp:GetAttribute("Slot")
				if not slotIndex or not plrData.Slots[slotIndex] then task.wait(1) continue end

				local storagePerks = plrData.Slots[slotIndex].Perks.Storage
				local equippedPerks = plrData.Slots[slotIndex].Perks.Equipped
				
				local sellRarities = Options.SellRaritiesDropdown.Value
				if sellRarities then
					for id, perkData in pairs(storagePerks) do
						local isEquipped = false
						for _, eqId in pairs(equippedPerks) do
							if eqId == id then isEquipped = true break end
						end
						
						if not isEquipped then
							local rarity = GetPerkRarity(perkData.Name)
							if rarity and sellRarities[rarity] then
								local success = pcall(function()
									return getRemote:InvokeServer("S_Equipment", "Sell", id)
								end)
								if success then
									Library:Notify({ Title = "Auto Sell", Description = "Sold " .. perkData.Name, Time = 1 })
								end
								task.wait(0.5)
							end
						end
...
UpgradesGroup:AddDropdown("SellRaritiesDropdown", {
...
-- Generate a sorted list of all perks for the dropdown with "None" at the top
local AllPerkNamesWithNone = {"None"}
local tempNames = {}
for rarity, names in pairs(Perks) do
...
table.sort(tempNames)
for _, name in ipairs(tempNames) do table.insert(AllPerkNamesWithNone, name) end

UpgradesGroup:AddToggle("AutoEquipPerkToggle", {
	Text = "Auto Equip Perk",
	Default = false,
})
Toggles.AutoEquipPerkToggle:OnChanged(function()
	getgenv().AutoEquipPerk = Toggles.AutoEquipPerkToggle.Value
	if getgenv().AutoEquipPerk then
		if game.PlaceId ~= 14916516914 then return end
		task.spawn(function()
			while getgenv().AutoEquipPerk do
				...
				local storagePerks = slot.Perks.Storage
				local equippedPerks = slot.Perks.Equipped
				
				-- Gather priorities in order
				local targetPerks = {}
				for _, opt in ipairs({
					...
					Options.PerkPriority5.Value
				}) do
					if opt and opt ~= "None" then
						table.insert(targetPerks, opt)
					end
				end

				-- Gather selected slots
				local selectedSlots = {}
				if Options.PerkSlotDropdown.Value then
					for slotName, isSelected in pairs(Options.PerkSlotDropdown.Value) do
						if isSelected then table.insert(selectedSlots, slotName) end
					end
				end

				if #selectedSlots == 0 or #targetPerks == 0 then
					task.wait(2)
					continue
				end

				-- Find highest priority perks we actually own
				local topOwnedPerks = {}
				local usedIds = {}
				for _, perkName in ipairs(targetPerks) do
					for id, perkData in pairs(storagePerks) do
						if perkData.Name == perkName and not usedIds[id] then
							table.insert(topOwnedPerks, {name = perkName, id = id})
							usedIds[id] = true
							break
						end
					end
					if #topOwnedPerks >= #selectedSlots then break end
				end

				local toEquip = {}
				local slotsToFill = {}
				for _, s in ipairs(selectedSlots) do slotsToFill[s] = true end

				-- Check which of the top perks are already correctly in one of our target slots
				local unassignedTopPerks = {}
				for _, p in ipairs(topOwnedPerks) do
					local alreadyInGoodSlot = false
					for s, _ in pairs(slotsToFill) do
						if equippedPerks[s] == p.id then
							alreadyInGoodSlot = true
							slotsToFill[s] = nil
							break
						end
					end
					if not alreadyInGoodSlot then
						table.insert(unassignedTopPerks, p)
					end
				end

				-- Assign remaining top perks to the remaining empty slots
				for _, p in ipairs(unassignedTopPerks) do
					local emptySlot = next(slotsToFill)
					if emptySlot then
						table.insert(toEquip, {id = p.id, slot = emptySlot, name = p.name})
						slotsToFill[emptySlot] = nil
					end
				end

				-- Equip ONE perk per loop tick to smoothly manage inventory
				if #toEquip > 0 then
					local e = toEquip[1]
					local success = pcall(function()
						return getRemote:InvokeServer("S_Equipment", "Equip", e.id, e.slot)
					end)
					
					if success then
						Library:Notify({
							Title = "Auto Equip",
							Description = "Equipped " .. e.name .. " to " .. e.slot,
							Time = 2
						})
						lastPlayerData = nil 
					end
...
UpgradesGroup:AddDropdown("PerkPriority1", { Values = AllPerkNamesWithNone, Default = 1, Multi = false, Text = "Priority 1 Perk" })
UpgradesGroup:AddDropdown("PerkPriority2", { Values = AllPerkNamesWithNone, Default = 1, Multi = false, Text = "Priority 2 Perk" })
UpgradesGroup:AddDropdown("PerkPriority3", { Values = AllPerkNamesWithNone, Default = 1, Multi = false, Text = "Priority 3 Perk" })
UpgradesGroup:AddDropdown("PerkPriority4", { Values = AllPerkNamesWithNone, Default = 1, Multi = false, Text = "Priority 4 Perk" })
UpgradesGroup:AddDropdown("PerkPriority5", { Values = AllPerkNamesWithNone, Default = 1, Multi = false, Text = "Priority 5 Perk" })

UpgradesGroup:AddLabel("Auto equips highest priority owned perks\ninto the selected slots.")
					...
					if familyRarity == "mythical" and getgenv().MythicalFamilyWebhook and webhook and webhook ~= "" then
						local payload = {
							...
							embeds = {{
								title = "Family Roll Success",
								color = 0xff0000,
								fields = {{ name = "Information", value = string.rep("`", 3) .. "\nUser: " .. lp.Name .. "\nFamily: " .. familyString .. "\n" .. string.rep("`", 3), inline = true }},
...
-- ==========================================
-- MISC TAB : Quests & Boosts
-- ==========================================

QuestsGroup:AddToggle("AutoQuestToggle", {
	Text = "Auto Claim Quests",
	Default = false,
})
Toggles.AutoQuestToggle:OnChanged(function()
	getgenv().AutoQuest = Toggles.AutoQuestToggle.Value
	if getgenv().AutoQuest then
		task.spawn(function()
			while getgenv().AutoQuest do
				pcall(function()
					getRemote:InvokeServer("Quests", "Claim_All")
				end)
				task.wait(10)
			end
		end)
...
ShopGroup:AddToggle("AutoBuyBoostsToggle", {
...
Toggles.AutoBuyBoostsToggle:OnChanged(function()
	...
	if getgenv().AutoBuyBoosts then
		task.spawn(function()
			while getgenv().AutoBuyBoosts do
				local boostsToBuy = Options.BoostsBuyDropdown.Value
				if boostsToBuy then
					for boostName, isSelected in pairs(boostsToBuy) do
						if isSelected then
							pcall(function()
								getRemote:InvokeServer("Shop", "Buy", boostName)
							end)
							task.wait(0.5)
						end
					end
				end
				task.wait(60) -- Checks the shop once a minute
			end
		end)
	end
end)

ShopGroup:AddDropdown("BoostsBuyDropdown", {
	Values = {"2x Gold Boost [30m]", "2x Gold Boost [1h]", "2x Gold Boost [2h]", "2x EXP Boost [30m]", "2x EXP Boost [1h]", "2x EXP Boost [2h]"},
	Default = {},
	Multi = true,
	Text = "Boosts to Buy",
})
