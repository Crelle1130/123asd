...
local function doAppoint()
	...
	task.wait(0.4);if not sf.Visible then return end
	...
	if #tr>0 then
		...
		if pcP then
			...
			if np then
				...
				task.wait(0.3);hm.Sit=false;task.wait(1.0)
			end
		end
		return
	else
...
local tak = ht:AddToggle({ Name="Anti AFK", Value=AAFK, Callback=function(v)AAFK=v;Per.AntiAFK=v end })
	local tbAP = ht:AddToggle({ Name="Auto Print (buy ink<20/paper<20)", Value=AP, Callback=function(v)AP=v;Per.AutoPrint=v end })
	local tbAB = ht:AddToggle({ Name="Auto Pay Bills", Value=AB, Callback=function(v)AB=v;Per.AutoBill=v end })
...
-- Buy task (only buys on restock: stock 0→positive)
local lastG, lastP = {}, {}
...
local function doPrint()
	...
	if pr then for _,p in ipairs(ob():GetDescendants()) do if p:IsA("ProximityPrompt")and(p.ObjectText or""):lower():match("printer")then pp=p;break end end end
	...
	if needInk or paperNeed then
		...
		if r2 then
			...
			if paperNeed then local cap=s.PaperCapacity or 80;local packs=math.ceil(cap/20);for _=1,packs do ASRQ:FireServer("Purchase","Copy Paper");task.wait(0.7)end end
			task.wait(0.5)
			h.CFrame=CFrame.new(pos.X,pos.Y+3,pos.Z)
		end
	end
...
-- Priority (higher wins): Fire > Appoint > Clean > Chef > Buy > Print.
...
local function doPayBillsWork()
	if not AB then return false end
	if not billState then return true end
	if not billState.PayableBill or billState.PayableBill<=0 then return false end
	return true
end
...
local SCHED = {
	{ name="Fire",   emergency=true, enabled=function() return AE end, hasWork=doFireWork,    run=doFire,    cooldown=1.0 },
	...
	{ name="Buy",    enabled=function() return BuyG or BuyP end, hasWork=function() return true end, run=doBuy, cooldown=4.0 },
	{ name="Bill",   enabled=function() return AB end, hasWork=doPayBillsWork, run=doPayBills, cooldown=15.0 },
